  ...
  list.innerHTML = clients.map(client => `
    <div class="saved-item ${client.id === state.editingClientId ? "active" : ""}">
      <div class="saved-item-main">
        ...
        <div class="saved-item-subtitle">${escapeHtml(client.accountId)} · ${escapeHtml(client.dateStart || "sem inicio")} ate ${escapeHtml(client.dateEnd || "sem fim")}</div>
      </div>
      <div class="saved-item-actions">
...
async function fetchAdAccounts() {
  ...
  try {
    ...
    populateAccountSelects();
    const hasSelected = state.selectedAccount && state.adAccounts.some(account => account.id === state.selectedAccount);
    selectAccount(hasSelected ? state.selectedAccount : (state.adAccounts[0]?.id || ""));
    updateMetaStatus(true);
  } catch (error) {
...
function populateAccountCards() {
  ...
  grid.innerHTML = state.adAccounts.map(account => `
...
function processDashboardData(dashboard) {
  ...
  renderLineChart(dashboard.daily?.labels || [], dashboard.daily?.addToCart || [], dashboard.daily?.purchases || []);
  renderCreatives();
  renderDonutChart();
}
...
function loadDemoData() {
  ...
  renderCampaignsTable();
  renderLineChart(DEMO_DATA.daily.labels, DEMO_DATA.daily.addToCart, DEMO_DATA.daily.purchases);
  renderCreatives();
  renderDonutChart();
}
...
function updateKPIs(data) {
  ...
  setText("kpi-compras", data.purchases);
  setText("kpi-cpp", `R$ ${fmtMoney(data.cpp)}`);
  ...
  setText("bm-cac", `R$ ${fmtMoney(data.addToCart > 0 ? data.spend / data.addToCart : 0)}`);
  setText("bm-cpp", `R$ ${fmtMoney(data.cpp)}`);
  setText("bm-cpm", `R$ ${fmtMoney(data.cpm)}`);
  ...
  setText("mm-roas", data.roas.toFixed(2));
  document.getElementById("roas-fill").style.width = `${Math.min((data.roas / 5) * 100, 100)}%`;
  setText("mm-conv", `R$ ${fmtMoney(data.convValue)}`);
}
...
function renderCampaignsTable() {
  ...
  tbody.innerHTML = state.campaigns.map((campaign, index) => `
    <tr>
      ...
      <td class="tag-highlight">${campaign.addToCart}</td>
      <td class="tag-highlight">${campaign.purchases}</td>
      <td>R$ ${fmtMoney(campaign.spend)}</td>
      <td style="color:var(--accent);font-weight:700">${campaign.roas.toFixed(2)}x</td>
    </tr>
  `).join("");
}
    ...
    <div class="creative-card">
      ...
      <div class="creative-info">
        ...
        <div class="creative-stats">
          <div class="creative-stat"><span>Carrinho</span><span>${creative.addToCart}</span></div>
          <div class="creative-stat"><span>Compras</span><span>${creative.purchases}</span></div>
          <div class="creative-stat"><span>Cliques</span><span>${creative.clicks}</span></div>
...
function renderLineChart(labels, addToCart, purchases) {
  destroyChart("line");
  ...
  state.charts.line = new Chart(ctx, {
    ...
    data: {
      labels,
      datasets: [
        { label: "Adicao de Carrinho", data: addToCart, borderColor: "#a8c441", backgroundColor: "rgba(168,196,65,0.05)", tension: 0.4, pointRadius: 3, pointBackgroundColor: "#a8c441", borderWidth: 2 },
        { label: "Compras", data: purchases, borderColor: "#ffffff", backgroundColor: "rgba(255,255,255,0.03)", tension: 0.4, pointRadius: 3, pointBackgroundColor: "#ffffff", borderWidth: 2 }
      ]
    },
    options: {
...
function renderDonutChart() {
  ...
  const data = state.creatives
    .filter(creative => creative.purchases > 0)
    .sort((a, b) => b.purchases - a.purchases)
    .slice(0, 5);
  const total = data.reduce((sum, creative) => sum + creative.purchases, 0);
  ...
  state.charts.donut = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        data: data.map(creative => creative.purchases),
  ...
  document.getElementById("donut-legend").innerHTML = data.map((creative, index) => `
    <div class="legend-item">
      ...
      <span class="legend-text">${escapeHtml(creative.name)} · ${((creative.purchases / total) * 100).toFixed(1)}%</span>
    </div>
  `).join("");
}
